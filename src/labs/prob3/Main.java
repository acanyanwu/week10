...
//	}
	...
	public static void test1() {
		List<String> list = Arrays.asList(null, "Bob", "Joe", "Tom");
   ...
   public static <T> boolean contains(List<? extends T> list, T searchValue, BiPredicate<? super T, ? super  T> predicate) {
        return list.stream()
            .map(entry-> {
                if(entry == null && searchValue == null) return true;
                if(entry == null || searchValue == null) return false;
                return predicate.test(entry, searchValue);
            })
            .anyMatch(outcome->outcome==true);
    }
